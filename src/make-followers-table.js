...
const GARY_QUORA_HTML = 'dist/GaryFollowersTable.html';
// 
// exampleCsvEntry = {
...
const HEADINGS = ['#', 'Name', 'Description', 'Followers' ];
...
const main = async () => {
    let html = `<head><meta charset="utf-8"><style>${NO_CSS}${CSS}</style></head><body>
    ...
    followers.forEach((follower, i) => {
        totalFollowers += parseFloat(follower.Followers);

        const cells = [];
        const num = '' + (i + 1);
        const nameLink = `<a class="name" href="${follower.Link}" target=_blank>${follower.Name}</a>`;

        cells.push(num);
        cells.push(nameLink);
        cells.push(follower.Description);
        cells.push(follower.Followers);
    ...
    fs.writeFile(GARY_QUORA_HTML, html, function (err) {
        ...
        console.log(`Total followers: ${totalFollowers}, average: ${1.0 * totalFollowers / followers.length}`);
    });
};
